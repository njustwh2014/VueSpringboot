<template>
  <div class="me-view-comment-item">
    <div class="me-view-comment-author">
      <a class="">
        <img class="me-view-picture" :src="comment.author.headportraiturl">
      </a>
      ...
      <div class="me-reply-list">
        <div class="me-reply-item" v-for="c in comment.childrens" :key="c.id">
          <div style="font-size: 14px">
            ...
            <span v-if="c.atlevel == 2" class="me-reply-user">@{{c.toUser.nickname}} </span>
    ...
    methods: {
      showComment(commentShowIndex, toUser) {
        ...
        if (this.commentShowIndex !== commentShowIndex) {
          ...
          if (toUser) {
            ...
            this.reply.toid=commentShowIndex
          } else {
      ...
      publishComment() {
        ...
        let replydata={
          ...
          content:that.reply.content,
          parentid:this.comment.id,
          toid:that.reply.toid
        }
        console.log("replydata:",replydata)
        publishComment(replydata).then(data => {
          that.$message({type: 'success', message: '评论成功', showClose: true})
          if(!that.comment.childrens){
            that.comment.childrens = []
          }
          that.comment.childrens.unshift(data.data.data)
